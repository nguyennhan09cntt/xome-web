    ...
    $.fn.gMap = function (options, methods_options) {
        ...
        switch (options) {
            ...
            case 'addMarkerInfo':
            {
                return $(this).trigger(
                    'gMap.addMarkerInfo',
                    [methods_options.latitude, methods_options.longitude, methods_options.content, methods_options.marker_content]
                );
            }
            case 'centerAt':
            ...
            case 'bound':
                return $(this).trigger('gMap.bound', methods_options);
        }
        ...
        return this.each(function () {
           ...
           // Bind bound
            $(this).bind('gMap.bound', function (e, option) {

                var bound = new google.maps.LatLngBounds();
                var length = option.markers.length;
                var zoom = 16;

                for (var i = 0; i < length; i++) {
                    bound.extend(new google.maps.LatLng(option.markers[i].latitude, option.markers[i].longitude));
                }


                if(length == 1) {
                    $gmap.setZoom(zoom);
                    $gmap.panTo(new google.maps.LatLng(option.markers[0].latitude, option.markers[0].longitude));
                }else{
            ...
            $(this).bind('gMap.addMarkerInfo', function (e, latitude, longitude, content, marker_content) {
                var marker_icon;
                var marker_shadow;
                var glatlng = new google.maps.LatLng(parseFloat(latitude), parseFloat(longitude));
